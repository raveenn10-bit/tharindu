...
export default function FeaturedWork({ onOpenProject }) {
  ...
  const infiniteMobileAlbums = [
    ...albums.map((item, idx) => ({ ...item, uniqueKey: `mob-a-${idx}-${item.id}` })),
    ...albums.map((item, idx) => ({ ...item, uniqueKey: `mob-b-${idx}-${item.id}` })),
  ]

  // Desktop: Show first 6 albums initially, or all albums when "View More" is clicked
  const displayedItems = showAll ? albums : albums.slice(0, 6)
        ...
        <div className="hidden md:block">
          ...
          {albums.length > 6 && (
            ...
            >
              ...
              >
                {showAll ? 'Show Less' : `View More (${albums.length})`}
              </button>
            </motion.div>
          )}
        </div>
